...
def remove_reasoning_from_output(text: str) -> str:
    ...
    if json_start != -1:
        # Substring from JSON start position
...
def extract_clean_response(text: str) -> Dict[str, Any]:
    ...
    print(f"Cannot parse JSON response: {cleaned_text[:200]}...")
    return {"error": "JSON parsing failed", "raw_text": cleaned_text}


def fix_incomplete_json(text: str) -> str:
    """
    Repair incomplete JSON response
    
    Args:
        text: Original text
        
    Returns:
        Repaired JSON text, or empty string if repair fails
    """
    ...
    # Verify if repaired JSON is valid
    try:
        json.loads(text)
        return text
    except JSONDecodeError:
...
def format_search_results_for_prompt(search_results: List[Dict[str, Any]], 
    ...
    Args:
        search_results: Search results list
        max_length: Max length of each result
