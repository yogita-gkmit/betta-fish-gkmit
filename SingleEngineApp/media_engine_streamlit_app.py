...
Provides a user-friendly Web interface for Media Agent
"""
...
def main():
    ...
    st.markdown("AI Agent with powerful multimodal capabilities")
    st.markdown("Break through traditional text communication limitations, widely browsing videos, graphics, and live streams from TikTok, Kuaishou, and Xiaohongshu")
    st.markdown("Further enhance capabilities using multimodal structured information provided by modern search engines, such as calendar cards, weather cards, stock cards, etc.")
    ...
    if start_research:
        ...
        # Since Gemini is forced, check relevant API keys
        ...
        # Automatically use API key from config file
        engine_key = settings.MEDIA_ENGINE_API_KEY
        bocha_key = settings.BOCHA_WEB_SEARCH_API_KEY

        # Build Settings (pydantic_settings style, uppercase environment variables take precedence)
        config = Settings(
