...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if config.ENABLE_CDP_MODE:
                utils.logger.info("[DouYinCrawler] Launching browser in CDP mode")
                ...
                utils.logger.info("[DouYinCrawler] Launching browser in standard mode")
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * dy_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                if "data" not in posts_res:
                    utils.logger.error(f"[DouYinCrawler.search] search douyin keyword: {keyword} failed, account might be blocked.")
                    break
    ...
    async def get_comments(self, aweme_id: str, semaphore: asyncio.Semaphore) -> None:
        async with semaphore:
            try:
                # Pass keyword list to get_aweme_all_comments method
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            utils.logger.error(f"[DouYinCrawler] CDP mode launch failed, falling back to standard mode: {e}")
    ...
    async def get_aweme_media(self, aweme_item: Dict):
        """
        Get Douyin media, automatically determine if media type is short video or post image and download

        Args:
            aweme_item (Dict): Douyin work details
        """
        if not config.ENABLE_GET_MEIDAS:
            utils.logger.info(f"[DouYinCrawler.get_aweme_media] Crawling image mode is not enabled")
            return
        # Note urls list, returns empty list if it is short video type
        note_download_url: List[str] = douyin_store._extract_note_image_list(aweme_item)
        # Video url, always exists, but for short video type the file is actually audio file
        video_download_url: str = douyin_store._extract_video_download_url(aweme_item)
        # TODO: Douyin does not use audio-video separation strategy, so audio can be separated from original video, not extracted for now
