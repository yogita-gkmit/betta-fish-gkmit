...
class BilibiliSign:
    ...
    def sign(self, req_data: Dict) -> Dict:
        """
        Add current timestamp to request parameters, sort keys lexicographically.
        Then URL encode request parameters, concatenate with salt, and md5 hash to generate w_rid parameter.
        :param req_data:
        :return:
        """
...
def parse_video_info_from_url(url: str) -> VideoUrlInfo:
    ...
    Args:
        url: Bilibili video link
            ...
            - BV1d54y1g7db (Pass BV ID directly)
    Returns:
        VideoUrlInfo: Object containing video ID
    """
    # If passed value is already a BV ID, return directly
    if url.startswith("BV"):
        return VideoUrlInfo(video_id=url)

    # Use regex to extract BV ID
...
def parse_creator_info_from_url(url: str) -> CreatorUrlInfo:
    ...
    Args:
        url: Bilibili creator space link
            ...
            - 434377496 (Pass UID directly)
    Returns:
        CreatorUrlInfo: Object containing creator ID
    """
    # If passed value is already a pure numeric ID, return directly
    if url.isdigit():
        return CreatorUrlInfo(creator_id=url)

    # Use regex to extract UID
    # Match /space.bilibili.com/digit format
    ...
    raise ValueError(f"Could not parse creator ID from URL: {url}")
