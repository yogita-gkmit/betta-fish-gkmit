...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if config.ENABLE_CDP_MODE:
                utils.logger.info("[BilibiliCrawler] Launching browser in CDP mode")
                ...
                utils.logger.info("[BilibiliCrawler] Launching browser in standard mode")
    ...
    async def get_pubtime_datetime(
        ...
        """
        Get Bilibili work publish date start timestamp pubtime_begin_s and publish date end timestamp pubtime_end_s
        ---
        :param start: publish date start, YYYY-MM-DD
        :param end: publish date end, YYYY-MM-DD

        Note
        ---
        - The search time range is from start to end, including start and end
        - To search for content on the same day, to include the search content of the start day, the value of pubtime_end_s should be the value of pubtime_begin_s plus one day minus one second, which is the last second of the start day
            - For example, if you only search for content on 2024-01-05, pubtime_begin_s = 1704384000, pubtime_end_s = 1704470399
              Converted to readable datetime object: pubtime_begin_s = datetime.datetime(2024, 1, 5, 0, 0), pubtime_end_s = datetime.datetime(2024, 1, 5, 23, 59, 59)
        - To search for content from start to end, to include the search content of the end day, the value of pubtime_end_s should be the value of pubtime_end_s plus one day minus one second, which is the last second of the end day
            - For example, search for content from 2024-01-05 to 2024-01-06, pubtime_begin_s = 1704384000, pubtime_end_s = 1704556799
              Converted to readable datetime object: pubtime_begin_s = datetime.datetime(2024, 1, 5, 0, 0), pubtime_end_s = datetime.datetime(2024, 1, 6, 23, 59, 59)
        """
        ...
        elif start_day == end_day:  # Search content of the same day
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * bili_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                videos_res = await self.bili_client.search_video_by_keyword(
                    ...
                    pubtime_begin_s=0,  # Work publish date start timestamp
                    pubtime_end_s=0,  # Work publish date end timestamp
                )
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            utils.logger.error(f"[BilibiliCrawler] CDP mode launch failed, falling back to standard mode: {e}")
    ...
    async def close(self):
        ...
        try:
            # If using CDP mode, special handling is needed
            if self.cdp_manager:
